...
@task
def test():
    sh('python -m coverage run --source src --omit="src/vendors/**,src/rental_price_finder_service.py" -m unittest discover -s test')
...
@task
def run():
    sh('python src/rental_price_finder_service.py')
