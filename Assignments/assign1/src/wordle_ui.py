...
def create_window():
    ...
    guess_btn_cmd = partial(guess_btn_action, cells, guess_btn, window)
    ...
    window.bind('<Return>', lambda *_: return_key_pressed(cells, guess_btn, window))
...
def return_key_pressed(cells, guess_btn, window):
    guess_btn_state = str(guess_btn["state"])
    if guess_btn_state == "normal":
        guess_btn_action(cells, guess_btn, window)


def guess_btn_action(cells, guess_btn, window):
    target_word = "FAVOR"
    guessed_word = ""
    ...
    result = play(target_word, guessed_word, current_row)
...
def main():
    window, entries, guess_btn = create_window()
    window.mainloop()
